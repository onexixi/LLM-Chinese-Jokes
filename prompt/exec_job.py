...
from prompt.though_prompt import rule_list, do_rule_prompt
...
def query_llm(prompt, max_retries=3, delay=20):
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                ...
                return f"Error: Unable to get response after {max_retries} attempts. Last error: {str(e)}"
...
async def execute_step(step_name, question, prompt):
    formatted_prompt = do_rule_prompt.format(question=question, step=step_name, rul_prompt=prompt)
...
async def process_yaml_content(yaml_data):
    results = {}
    for key, value in yaml_data.items():
        if isinstance(value, dict) and '描述' in value:
            question =key+"\n描述："+value['描述']+'\n具体执行步骤:'+str(value['具体执行步骤'])
            print(f"处理问题: {question}")
            step_results, overall_guidance = await problem_solving_framework(question)
            results[key] = {
                '问题': question,
                '步骤结果': step_results,
                '总体建议': overall_guidance
            }
    return results


async def main():
    yaml_file_path = 'mind.yaml'
    yaml_data = read_yaml_file(yaml_file_path)

    results = await process_yaml_content(yaml_data)

    with open("problem_solving_results.txt", "w", encoding="utf-8") as f:
        for key, value in results.items():
            f.write(f"主题: {key}\n")
            f.write(f"问题: {value['问题']}\n\n")

            for step_name, method_results in value['步骤结果']:
